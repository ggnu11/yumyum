...
export class PinService {
        ...
        async getPinById(id: number, user: User) {
            try {
                const foundPost = await this.pinRepository
                    .createQueryBuilder('pin')
                    .leftJoinAndSelect('pin.images', 'image')
                    .leftJoinAndSelect(
                        'pin.wishlists',
                ...
                const { wishlists, images, ...rest } = foundPost;
                const pinWithWishlists = {
                    ...rest,
                    isFavorite: wishlists.length > 0,
                    imageUris: images,
                };
    
                return pinWithWishlists;
            } catch (error) {
                console.log(error);
                throw new InternalServerErrorException(
