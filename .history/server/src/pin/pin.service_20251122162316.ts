...
export class PinService {
        ...
        async getPostsByMonth(year: number, month: number, user: User) {
            const posts = await this.pinRepository
                .createQueryBuilder('pin')
                .where('pin.user_id = :user_id', { user_id: user.user_id })
                .andWhere('extract(year from pin.date) = :year', { year })
                .andWhere('extract(month from pin.date) = :month', { month })
                .select([
                    'pin.id AS id',
                    'pin.title AS title',
                    'pin.address AS address',
                    'EXTRACT(DAY FROM pin.visit_date) AS date',
                ])
                .getRawMany();
