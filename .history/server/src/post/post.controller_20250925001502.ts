...
export class PostController {
  ...
  @ApiResponse({ status: 200, description: 'Markers successfully retrieved.' })
  getAllMarkers(@GetUser() user: User) {
    return this.postService.getAllMarkers(user);
  }

  @Get('/posts')
  @ApiOperation({ summary: 'Get posts (my posts with page, or by year/month)' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number for my posts' })
